...
function Build-WslTestBinaries {
    ...
    [IO.File]::WriteAllText($manifestPath, "", (New-Object System.Text.UTF8Encoding($false)))
    $oldGoOS = $env:GOOS
    $oldGoArch = $env:GOARCH
    $oldCGO = $env:CGO_ENABLED
    try {
        ...
        $env:GOARCH = "amd64"
        $env:CGO_ENABLED = "0"
        $packages = @(& go list -f '{{if or .TestGoFiles .XTestGoFiles}}{{.ImportPath}}{{end}}' ./cmd/... ./internal/... | Where-Object { -not [string]::IsNullOrWhiteSpace($_) })
        ...
        for ($index = 0; $index -lt $packages.Count; $index++) {
            ...
            $relativeDirectory = $package -replace '^asterferry/', ''
            Add-Content -Encoding utf8 -Path $manifestPath -Value ("{0:D3}`t{1}`t{2}" -f $index, $package, $relativeDirectory)
        }
    } finally {
        if ($null -eq $oldGoOS) { Remove-Item Env:GOOS -ErrorAction SilentlyContinue } else { $env:GOOS = $oldGoOS }
        if ($null -eq $oldGoArch) { Remove-Item Env:GOARCH -ErrorAction SilentlyContinue } else { $env:GOARCH = $oldGoArch }
...
try {
    ...
    Invoke-Logged "Windows installer syntax and contract checks" "pwsh" @("-NoProfile", "-File", (Join-Path $root "scripts/test-installers.ps1"))
    ...
    Invoke-Logged "Windows go vet" "go" @("vet", "./cmd/...", "./internal/...")
    Invoke-Logged "Windows staticcheck" "staticcheck" @("-checks=all,-SA1019", "./cmd/...", "./internal/...")
    Invoke-Logged "Windows vulnerability check" "govulncheck" @("./cmd/...", "./internal/...")
    Invoke-Logged "Windows full tests" "go" @("test", "-count=1", "./cmd/...", "./internal/...")
    ...
    if ($SkipRace) {
        ...
        Invoke-Logged "Windows race tests" "go" @("test", "-race", "-count=1", "./cmd/...", "./internal/...")
    }
