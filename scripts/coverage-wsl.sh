...
mapfile -t runtime_packages < <(go list -f '{{if .GoFiles}}{{.ImportPath}}{{end}}' ./cmd/... ./internal/...)
filtered_packages=()
...
go test -count=1 -covermode=atomic "-coverpkg=$coverpkg" "-coverprofile=$profile_path" ./cmd/... ./internal/...
