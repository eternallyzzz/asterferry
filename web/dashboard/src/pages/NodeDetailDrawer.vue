...
const installScriptSource = ref<NodeInstallScriptSource>("controller");
...
const nodeDecommissioned = computed(() => props.node?.certificate_state === "decommissioned");
...
async function loadGatewayNodes(nodeID: string) {
  ...
  try {
    ...
    gatewayNodes.value = result.items.filter((item) => item.id !== nodeID && item.enabled && item.certificate_state === "active");
  } catch (caught) {
    if (requestVersion !== gatewayRequestVersion || props.node?.id !== nodeID) return;
...
function openInstall() {
  ...
  installScriptSource.value = "controller";
  installError.value = "";
...
async function generateInstallCommand() {
  ...
  installError.value = "";
  try {
    const input = { platform: installPlatform.value, arch: installArch.value, script_source: installScriptSource.value };
      ...
      <div class="section-content">
        <PanelCard v-show="section === 'info'" title="基本信息">
          <dl class="info-grid">
            ...
            <div><dt>状态</dt><dd><StatusPill :tone="nodeDecommissioned ? 'bad' : node.enabled ? 'good' : 'neutral'">{{ nodeDecommissioned ? "已退役" : node.enabled ? "启用" : "停用" }}</StatusPill></dd></div>
          ...
          <div v-if="session.canOperate.value && !nodeDecommissioned" class="action-row">
          ...
          <div v-if="session.canAdmin.value" class="action-row">
            ...
            <button type="button" class="af-button secondary" @click="openInstall">{{ nodeDecommissioned ? "生成替换注册命令" : "生成安装命令" }}</button>
          </div>
        </PanelCard>
        ...
        <PanelCard v-show="section === 'spec'">
          ...
          <template v-else>
            ...
            <p v-if="nodeDecommissioned" class="form-note">节点已退役。请在“信息”中生成替换注册命令；执行后会保留原节点 ID 和规格，无需卸载二进制。</p>
            <p v-else-if="!nodeEnrolled" class="form-note">节点尚未完成注册。请先在目标机器执行安装注册命令，注册成功后再配置角色。</p>
      ...
      <div class="install-dialog">
        <template v-if="!installResult">
          <div class="install-grid">
            ...
            <label>安装脚本来源<select v-model="installScriptSource"><option value="controller">Controller（推荐）</option><option value="github">GitHub Release</option></select></label>
          </div>
          <p class="form-note">{{ nodeDecommissioned ? "这是替换注册命令：会停止旧服务、备份并清理旧身份和缓存，再使用新 Token 重新申请证书；不需要卸载二进制。" : "安装命令只安装并注册 Node daemon，不预设 Gateway 或 Agent。完成注册后，在“规格”中选择行为并保存。" }}</p>
        ...
        <template v-else>
          ...
          <p class="form-note">请在对应机器的管理员 PowerShell 或 root shell 中执行。命令包含一次性 Token，请勿公开。</p>
        </template>
      </div>
      <template #footer>
