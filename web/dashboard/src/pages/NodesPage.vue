...
const form = ref({ name: "", labels: "{}", enabled: true, platform: "linux" as "linux" | "windows", arch: "amd64" as "amd64" | "arm64", scriptSource: "controller" as NodeInstallScriptSource });
const editID = ref("");
const editRevision = ref(0);
const formError = ref("");
const saving = ref(false);
const pendingDelete = ref<ControllerNode | null>(null);
const pendingPurge = ref<ControllerNode | null>(null);
...
function openCreate() {
  form.value = { name: "", labels: "{}", enabled: true, platform: "linux", arch: "amd64", scriptSource: "controller" };
  editID.value = "";
...
function openEdit(node: ControllerNode) {
  form.value = { name: node.name, labels: prettyJson(node.labels || {}), enabled: node.enabled, platform: "linux", arch: "amd64", scriptSource: "controller" };
  ...
  try {
    ...
    if (editID.value) {
      ...
      const bootstrap = await createNodeInstallation({
        ...
        script_source: form.value.scriptSource,
      }, undefined, newIdempotencyKey());
      notify.success(`安装任务 ${bootstrap.node_id} 已创建；节点执行命令后才会注册。`);
...
async function confirmDelete() {
  ...
  try {
    ...
    notify.success(`节点 ${pendingDelete.value.id} 已退役；原机器可使用替换注册命令重新加入。`);
...
async function confirmPurge() {
  ...
  purging.value = true;
  try {
    await purgeNode(pendingPurge.value.id, pendingPurge.value.revision, undefined, newIdempotencyKey());
    notify.success(`节点 ${pendingPurge.value.id} 已永久删除。`);
  ...
  <div class="page-stack">
    ...
    <PanelCard :title="`节点清单 · ${nodes.length}`">
      ...
      <DataTable v-else :empty="!nodes.length">
        <thead>
          <tr><th>节点</th><th>行为规格</th><th>证书</th><th>状态</th><th>操作</th></tr>
        </thead>
        <tbody>
          <tr v-for="node in nodes" :key="node.id" class="clickable" @click="openDrawer(node)">
            ...
            <td><StatusPill :tone="node.certificate_state === 'decommissioned' ? 'bad' : node.enabled ? 'good' : 'neutral'">{{ node.certificate_state === 'decommissioned' ? "已退役" : node.enabled ? "启用" : "停用" }}</StatusPill></td>
            <td>
              <div class="row-actions" @click.stop>
                ...
                <template v-if="session.canOperate.value && node.certificate_state !== 'decommissioned'">
                ...
                <button v-if="session.canAdmin.value && node.certificate_state !== 'decommissioned'" type="button" class="af-button danger-text" @click="pendingDelete = node">退役</button>
                <button v-if="session.canAdmin.value && node.certificate_state === 'decommissioned'" type="button" class="af-button danger-text" @click="pendingPurge = node">永久删除</button>
              </div>
            </td>
          </tr>
        </tbody>
    ...
    <ModalDialog :open="formOpen" :title="editID ? '编辑节点' : '生成节点安装命令'" width="480px" @close="formOpen = false">
      <form class="form-stack" @submit.prevent="save">
        <template v-if="!editID">
          ...
          <FormField label="安装脚本来源">
            <select v-model="form.scriptSource">
              <option value="controller">Controller（推荐）</option>
        ...
        <p v-else class="form-note">Node ID 由 Controller 自动生成并绑定到本次安装任务。这里只创建待安装任务，不会提前注册节点；目标机器完成注册后，再在节点详情中选择 Gateway 或 Agent 行为并配置规格。Controller 来源从控制面下载脚本；GitHub 来源直接从发布页下载脚本。</p>
      </form>
      <template #footer>
    ...
    <ModalDialog :open="installOpen" title="一键安装注册命令" width="680px" @close="installOpen = false">
      <div v-if="installResult" class="install-dialog">
        <p class="form-note">Node daemon · {{ installResult.platform }}/{{ installResult.arch }} · v{{ installResult.version }} · {{ installResult.script_source === "controller" ? "Controller 脚本" : "GitHub 脚本" }}</p>
    ...
    <ModalDialog :open="Boolean(pendingDelete)" title="退役节点" @close="pendingDelete = null">
      <p class="confirm-text">确定退役节点 <strong>{{ pendingDelete?.id }}</strong> 吗？节点会停止运行并撤销旧证书，Service 会保留；原机器可使用新的注册命令重新加入，无需卸载。</p>
      <template #footer>
        <button type="button" class="af-button secondary" @click="pendingDelete = null">取消</button>
        <button type="button" class="af-button danger" :disabled="deleting" @click="confirmDelete">{{ deleting ? "退役中…" : "确认退役" }}</button>
      </template>
    </ModalDialog>

    <ModalDialog :open="Boolean(pendingPurge)" title="永久删除节点" @close="pendingPurge = null">
      <p class="confirm-text">确定永久删除节点 <strong>{{ pendingPurge?.id }}</strong> 吗？该操作不可恢复，会删除节点身份、证书、规格和运行状态，并使原机器上的节点永久无法凭旧证书重新接入。</p>
      <p class="form-note warning">永久删除不会自动删除 Service 或 Assignment。存在依赖时 Controller 会拒绝操作，请先处理这些业务资源。</p>
      <template #footer>
        <button type="button" class="af-button secondary" @click="pendingPurge = null">取消</button>
        <button type="button" class="af-button danger" :disabled="purging" @click="confirmPurge">{{ purging ? "删除中…" : "确认永久删除" }}</button>
      </template>
    </ModalDialog>
