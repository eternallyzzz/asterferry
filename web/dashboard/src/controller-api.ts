...
export type NodeInstallScriptSource = "controller" | "github";

export interface NodeBootstrapRequest {
  platform: "linux" | "windows";
  arch: "amd64" | "arm64";
  script_source: NodeInstallScriptSource;
}
...
export function purgeNode(id: string, revision: number, token?: string, idempotencyKey?: string): Promise<void> { return request<void>(`/nodes/${encodeURIComponent(id)}/actions/purge`, { method: "POST", headers: { "If-Match": String(revision), ...(idempotencyKey ? { "Idempotency-Key": idempotencyKey } : {}) } }, token); }
...
export interface ControllerObservedState {
  ...
  system_info?: ControllerSystemInfo;
  observed_at: string;
}
