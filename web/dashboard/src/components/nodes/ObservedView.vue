...
const systemInfoStale = computed(() => {
  const collectedAt = state.value?.system_info?.collected_at;
  return !collectedAt || Date.now() - new Date(collectedAt).getTime() > 90_000;
});

function systemValue(value: string | number | undefined): string {
  return value === undefined || value === "" || (typeof value === "number" && !Number.isFinite(value)) ? "—" : String(value);
}
  ...
  <div v-else-if="state" class="observed">
    ...
    <section class="system-info">
      <div class="runtime-heading">
        <div>
          <h3 class="section-title">机器信息</h3>
          <p class="runtime-note">节点最近一次上报的主机和资源概览。</p>
        </div>
        <StatusPill :tone="systemInfoStale ? 'warn' : 'good'">{{ systemInfoStale ? "信息已过期" : "实时" }}</StatusPill>
      </div>
      <EmptyState v-if="!state.system_info" title="暂无系统信息" description="节点完成连接后会自动上报主机信息。" />
      <dl v-else class="info-grid">
        <div><dt>主机名</dt><dd>{{ systemValue(state.system_info.hostname) }}</dd></div>
        <div><dt>系统</dt><dd>{{ systemValue(state.system_info.distribution || state.system_info.os) }} {{ systemValue(state.system_info.distribution_version) }}</dd></div>
        <div><dt>内核 / 架构</dt><dd>{{ systemValue(state.system_info.kernel) }} · {{ systemValue(state.system_info.architecture) }}</dd></div>
        <div><dt>CPU</dt><dd>{{ systemValue(state.system_info.cpu_model) }} · {{ systemValue(state.system_info.logical_cpus) }} 核</dd></div>
        <div><dt>CPU 使用率</dt><dd>{{ state.system_info.cpu_usage_percent === undefined ? "—" : `${state.system_info.cpu_usage_percent.toFixed(1)}%` }}</dd></div>
        <div><dt>运行时长</dt><dd>{{ formatDuration(state.system_info.uptime_seconds) }}</dd></div>
        <div><dt>内存</dt><dd>{{ formatBytes(state.system_info.memory_available_bytes) }} 可用 / {{ formatBytes(state.system_info.memory_total_bytes) }}</dd></div>
        <div><dt>磁盘</dt><dd>{{ formatBytes(state.system_info.disk_free_bytes) }} 可用 / {{ formatBytes(state.system_info.disk_total_bytes) }}</dd></div>
        <div><dt>IP 地址</dt><dd>{{ state.system_info.ip_addresses?.join(", ") || "—" }}</dd></div>
        <div><dt>Node / Go</dt><dd>{{ systemValue(state.system_info.node_version) }} · {{ systemValue(state.system_info.go_version) }}</dd></div>
        <div><dt>服务模式</dt><dd>{{ systemValue(state.system_info.service_mode) }}</dd></div>
        <div><dt>采集时间</dt><dd>{{ formatTime(state.system_info.collected_at) }}</dd></div>
      </dl>
    </section>
