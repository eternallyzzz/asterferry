...
func TestControllerGatewayAgentQUICEndToEnd(t *testing.T) {
	...
	waitFor(t, ctx, func() bool {
		for _, nodeID := range []string{"gateway-e2e", "agent-e2e"} {
			observed, observedErr := resources.GetObserved(ctx, nodeID)
			if observedErr != nil || observed.SystemInfo == nil || observed.SystemInfo.OS == "" || observed.SystemInfo.Architecture == "" || observed.SystemInfo.Hostname == "" || observed.SystemInfo.CollectedAt.IsZero() {
				return false
			}
		}
		return true
