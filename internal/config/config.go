...
type ManagementConfig struct {
	...
	// AuthTokenFile is retained as a migration input for bundles generated by
	// older releases. New configurations must use management.auth.
	AuthTokenFile string              `yaml:"auth_token_file"`
	AuthToken     []byte              `yaml:"-"`
	AdminToken    []byte              `yaml:"-"`
	ViewerToken   []byte              `yaml:"-"`
...
// intentionally point at the same file for legacy configurations, but init
// generates separate credentials.
...
func (c *Config) Validate() error {
	...
	legacyTokenFile := strings.TrimSpace(c.Management.AuthTokenFile)
	...
	previousAdminTokenFile := adminTokenFile
	if legacyTokenFile != "" && (adminTokenFile == "" || legacyTokenFile != adminTokenFile) {
		// Older callers may update AuthTokenFile after a Config has already
		// been validated. Treat that explicit legacy change as authoritative
		// and keep the viewer token aligned for the legacy single-token mode.
		adminTokenFile = legacyTokenFile
		if viewerTokenFile == "" || viewerTokenFile == previousAdminTokenFile {
			viewerTokenFile = legacyTokenFile
		}
	}
	if adminTokenFile == "" {
		adminTokenFile = legacyTokenFile
	...
	if viewerTokenFile == "" {
		viewerTokenFile = adminTokenFile
	}
	...
	// Keep the normalized admin path available to older callers until the
	// migration window is removed.
	c.Management.AuthTokenFile = c.Management.Auth.AdminTokenFile
