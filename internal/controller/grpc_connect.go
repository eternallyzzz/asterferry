...
func (s *ControlServer) Connect(stream v1.Control_ConnectServer) (returnErr error) {
	...
	if node.CertificateState == domain.CertificateDecommissioned {
		return status.Error(codes.PermissionDenied, "node is decommissioned")
	}
	if !node.Enabled {
		return status.Error(codes.PermissionDenied, "node is disabled")
	}
	if node.CertificateState != domain.CertificateActive {
		return status.Error(codes.PermissionDenied, "node certificate is not active")
	}
	...
	for {
		...
		case message.GetHeartbeat() != nil:
			...
			if currentNode.CertificateState == domain.CertificateDecommissioned {
				return status.Error(codes.PermissionDenied, "node is decommissioned")
			}
			...
			if previous, previousErr := s.resources.GetObserved(stream.Context(), hello.GetNodeId()); previousErr == nil {
				...
				observed.SystemInfo = previous.SystemInfo
				if !observed.Healthy {
