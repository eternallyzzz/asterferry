...
func loadObservedStateNormalized(ctx context.Context, q sqlQueryer, nodeID string) (domain.ObservedState, string, error) {
	...
	var geoipUp int
	if err := q.QueryRowContext(ctx, `SELECT node_id,generation,protocol_version,healthy,degraded,last_error_code,last_error_path,last_error_message,system_info_json,observed_at,updated_at,active_streams,active_sessions,active_egress,udp_oversize_drops,geoip_up,active_connections,active_flows,runtime_bytes_in_total,runtime_bytes_out_total,runtime_opened_total,runtime_closed_total,runtime_rejected_total,runtime_rate_limited_total,runtime_telemetry_dropped_total FROM observed_states WHERE node_id=?`, nodeID).Scan(&state.NodeID, &generation, &protocolVersion, &healthy, &degraded, &lastCode, &lastPath, &lastMessage, &systemInfoJSON, &observedAt, &updatedAt, &metrics[0], &metrics[1], &metrics[2], &metrics[3], &geoipUp, &metrics[4], &metrics[5], &metrics[6], &metrics[7], &metrics[8], &metrics[9], &metrics[10], &metrics[11], &metrics[12]); err != nil {
	...
	if len(systemInfoJSON) > 0 {
		...
		if err := jsonutil.DecodeStrict(systemInfoJSON, &systemInfo); err != nil {
			return domain.ObservedState{}, "", fmt.Errorf("decode stored system info: %w", err)
		}
...
func writeObservedStateTx(ctx context.Context, tx *sql.Tx, state domain.ObservedState, updatedAt time.Time) error {
	...
	if state.SystemInfo != nil {
		var err error
		systemInfoJSON, err = json.Marshal(state.SystemInfo)
		if err != nil {
			return err
		}
	}
	args := []any{state.NodeID, state.AppliedGeneration, state.SchemaVersion, boolInt(state.Healthy), boolInt(state.Degraded), lastCode, lastPath, lastMessage, systemInfoJSON, observedAt.Format(time.RFC3339Nano), updatedAt.Format(time.RFC3339Nano), metricSQL[0], metricSQL[1], metricSQL[2], metricSQL[3], boolInt(state.Metrics.GeoIPUp), metricSQL[4], metricSQL[5], metricSQL[6], metricSQL[7], metricSQL[8], metricSQL[9], metricSQL[10], metricSQL[11], metricSQL[12]}
	if _, err := tx.ExecContext(ctx, `INSERT INTO observed_states(node_id,generation,protocol_version,healthy,degraded,last_error_code,last_error_path,last_error_message,system_info_json,observed_at,updated_at,active_streams,active_sessions,active_egress,udp_oversize_drops,geoip_up,active_connections,active_flows,runtime_bytes_in_total,runtime_bytes_out_total,runtime_opened_total,runtime_closed_total,runtime_rejected_total,runtime_rate_limited_total,runtime_telemetry_dropped_total) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(node_id) DO UPDATE SET generation=excluded.generation,protocol_version=excluded.protocol_version,healthy=excluded.healthy,degraded=excluded.degraded,last_error_code=excluded.last_error_code,last_error_path=excluded.last_error_path,last_error_message=excluded.last_error_message,system_info_json=excluded.system_info_json,observed_at=excluded.observed_at,updated_at=excluded.updated_at,active_streams=excluded.active_streams,active_sessions=excluded.active_sessions,active_egress=excluded.active_egress,udp_oversize_drops=excluded.udp_oversize_drops,geoip_up=excluded.geoip_up,active_connections=excluded.active_connections,active_flows=excluded.active_flows,runtime_bytes_in_total=excluded.runtime_bytes_in_total,runtime_bytes_out_total=excluded.runtime_bytes_out_total,runtime_opened_total=excluded.runtime_opened_total,runtime_closed_total=excluded.runtime_closed_total,runtime_rejected_total=excluded.runtime_rejected_total,runtime_rate_limited_total=excluded.runtime_rate_limited_total,runtime_telemetry_dropped_total=excluded.runtime_telemetry_dropped_total`, args...); err != nil {
		return err
	}
