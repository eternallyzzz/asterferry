...
func (s *ResourceRepository) IssueNodeCertificate(ctx context.Context, config Config, token, nodeID string, csrDER []byte) (Certificate, error) {
	if err := domain.ValidateID(nodeID, "node_id"); err != nil {
		return Certificate{}, fmt.Errorf("%w: %w", ErrInvalidEnrollmentRequest, err)
	}
	boundNodeID, bound, err := parseNodeEnrollmentToken(token)
	if err != nil {
		return Certificate{}, fmt.Errorf("%w: %w", ErrInvalidEnrollmentRequest, err)
	}
	if bound && boundNodeID != nodeID {
	...
	replacement := bound && node.CertificateState == domain.CertificateDecommissioned
	if (!node.Enabled && !replacement) || node.CertificateState == domain.CertificateRevoked || (node.CertificateState == domain.CertificateDecommissioned && !replacement) {
	...
	defer tx.Rollback()
	// Re-enrollment changes the availability of every placement that references
	// this identity. Notify both endpoints after commit so a replacement Agent
	// or Gateway triggers the normal scheduler repair pass automatically.
	affectedNodes, affectedErr := assignmentParticipantIDsTx(ctx, tx, nodeID)
	if affectedErr != nil {
		return Certificate{}, storageFailure("load enrollment assignment participants", affectedErr)
	}
	...
	replacement = bound && certificateState == domain.CertificateDecommissioned
	if (enabled == 0 && !replacement) || certificateState == domain.CertificateRevoked || (certificateState == domain.CertificateDecommissioned && !replacement) {
		return Certificate{}, fmt.Errorf("%w: node is disabled", ErrNodeEnrollmentNotAllowed)
	}
	if err := validateCSRIdentity(request, nodeID); err != nil {
		return Certificate{}, fmt.Errorf("%w: %w", ErrInvalidEnrollmentRequest, err)
	...
	if replacement {
		result, err = tx.ExecContext(ctx, `UPDATE nodes SET enabled=1,certificate_state=?,certificate_serial=?,revision=?,updated_at=? WHERE id=? AND revision=? AND certificate_state=?`, domain.CertificateActive, certificate.Serial, revision+1, updatedAt, nodeID, revision, domain.CertificateDecommissioned)
	} else {
	...
	s.ChangeBus().notifyResourceChanges(affectedNodes...)
...
func (s *ResourceRepository) RenewNodeCertificate(ctx context.Context, config Config, nodeID string, csrDER []byte) (Certificate, error) {
	if err := domain.ValidateID(nodeID, "node_id"); err != nil {
		return Certificate{}, fmt.Errorf("%w: %w", ErrInvalidEnrollmentRequest, err)
	}
	node, err := s.GetNode(ctx, nodeID)
