...
func loadObservedBatchNormalized(ctx context.Context, q sqlQueryer, gateways []domain.Node) (map[string]domain.ObservedState, error) {
	...
	for start := 0; start < len(nodeIDs); {
		...
		for rows.Next() {
			...
			if err := rows.Scan(&state.NodeID, &generation, &protocolVersion, &healthy, &degraded, &lastCode, &lastPath, &lastMessage, &systemInfoJSON, &observedAt, &updatedAt, &metrics[0], &metrics[1], &metrics[2], &metrics[3], &geoipUp, &metrics[4], &metrics[5], &metrics[6], &metrics[7], &metrics[8], &metrics[9], &metrics[10], &metrics[11], &metrics[12]); err != nil {
				_ = rows.Close()
				return nil, err
			}
			...
			if len(systemInfoJSON) > 0 {
				...
				if err := jsonutil.DecodeStrict(systemInfoJSON, &systemInfo); err != nil {
					_ = rows.Close()
					return nil, fmt.Errorf("decode stored system info: %w", err)
				}
