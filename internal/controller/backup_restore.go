...
func Restore(config Config, source, destination string) error {
	...
	for _, name := range backupNodeInstallerFiles {
		if fileExists(filepath.Join(source, name)) {
			if err := copyFile(filepath.Join(source, name), filepath.Join(destination, "node-installers", name), 0o600); err != nil {
				return fmt.Errorf("restore Node installer asset %q: %w", name, err)
			}
		}
	}
	if backupConfig := filepath.Join(source, "controller.json"); fileExists(backupConfig) {
		...
		restored.NodeInstallersDir = filepath.Join(destination, "node-installers")
