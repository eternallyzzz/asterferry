...
func (s *Server) nodeInstallations(w http.ResponseWriter, r *http.Request) {
	...
	if _, _, err := validateBootstrapConfiguration(s.config, input.ScriptSource); err != nil {
		writeError(w, http.StatusServiceUnavailable, "bootstrap_unavailable", err.Error())
		return
	}

	plain, pending, err := s.resources.CreatePendingNodeBootstrapWithSource(r.Context(), node, platform, arch, input.ScriptSource, WriteOptions{Actor: user.Username, IdempotencyKey: r.Header.Get("Idempotency-Key")})
	if err != nil {
	...
	response, err := buildNodeInstallCommand(s.config, node, platform, arch, pending.ScriptSource, plain, nil)
	if err != nil {
		writeError(w, http.StatusServiceUnavailable, "bootstrap_unavailable", err.Error())
		return
	}
...
func (s *Server) nodeInstallationAction(w http.ResponseWriter, r *http.Request) {
	...
	if len(parts) == 2 && parts[1] == "reissue" {
		...
		pendingForValidation, pendingErr := s.resources.GetPendingNodeBootstrap(r.Context(), nodeID)
		if pendingErr != nil {
			writeStoreError(w, pendingErr)
			return
		}
		if _, _, err := validateBootstrapConfiguration(s.config, pendingForValidation.ScriptSource); err != nil {
			writeError(w, http.StatusServiceUnavailable, "bootstrap_unavailable", err.Error())
			return
		}
		...
		response, err := buildNodeInstallCommand(s.config, pending.node(), pending.Platform, pending.Arch, pending.ScriptSource, plain, nil)
		if err != nil {
			writeError(w, http.StatusServiceUnavailable, "bootstrap_unavailable", err.Error())
			return
		}
