...
func verifyBackupManifest(directory string, payloadFiles []string) (backupManifest, error) {
	...
	allowed := make(map[string]struct{}, len(expected)+len(backupNodeInstallerFiles))
	...
	if len(manifest.Files) < len(expected) || len(manifest.Files) > len(allowed) {
		return backupManifest{}, errors.New("backup manifest does not contain a valid payload set")
	}
	seen := make(map[string]struct{}, len(manifest.Files))
	for _, entry := range manifest.Files {
		if _, ok := allowed[entry.Name]; !ok || strings.ContainsAny(entry.Name, `/\\`) {
