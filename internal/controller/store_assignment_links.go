...
// detachServiceFromAssignmentTx removes a service from its scheduler-owned
// assignment as part of the same transaction that deletes the service. An
// assignment containing no services is removed entirely; an assignment with
// other services keeps its identity but receives a new generation so both
// participating nodes retire the deleted service atomically.
func detachServiceFromAssignmentTx(ctx context.Context, tx *sql.Tx, serviceID, actor string) ([]string, error) {
	assignment, assigned, err := assignmentForService(ctx, tx, serviceID)
	if err != nil {
		return nil, err
	}
	if !assigned {
	...
	remainingServices := make([]string, 0, len(assignment.ServiceIDs))
	for _, candidate := range assignment.ServiceIDs {
		if candidate != serviceID {
			remainingServices = append(remainingServices, candidate)
		}
	}
	remainingBindings := make([]domain.Binding, 0, len(assignment.Bindings))
	...
	if len(remainingServices) == 0 {
		result, err := tx.ExecContext(ctx, `DELETE FROM assignments WHERE id=? AND revision=?`, assignment.ID, assignment.Revision)
		if err != nil {
			return nil, err
		}
		if err := requireRevisionWrite(ctx, tx, result, "assignment", assignment.Revision, `SELECT revision FROM assignments WHERE id=?`, assignment.ID); err != nil {
			return nil, err
		}
		if err := insertAudit(ctx, tx, actor, "delete", "assignment", assignment.ID, assignment.Revision, map[string]string{"reason": "service_deleted", "service_id": serviceID}); err != nil {
			return nil, err
		}
		return affectedNodes, nil
	}
	if assignment.Generation == math.MaxUint64 {
		return nil, &domain.ApplyError{Code: "invalid_generation", Path: "assignment.generation", Message: "assignment generation is exhausted"}
	}
	...
	assignment.ServiceIDs = remainingServices
	assignment.Bindings = remainingBindings
	assignment.Generation++
	if assignment.State != domain.AssignmentDegraded && assignment.State != domain.AssignmentDraining {
		assignment.State = domain.AssignmentPending
	}
	...
	assignment.Revision++
	assignment.UpdatedAt = time.Now().UTC()
	result, err := updateAssignmentColumnsTx(ctx, tx, assignment, previousRevision)
	if err != nil {
		return nil, err
	}
	if err := requireRevisionWrite(ctx, tx, result, "assignment", previousRevision, `SELECT revision FROM assignments WHERE id=?`, assignment.ID); err != nil {
		return nil, err
	}
	...
	if err := insertAudit(ctx, tx, actor, "derived_service", "assignment", assignment.ID, assignment.Revision, map[string]string{"reason": "service_deleted", "service_id": serviceID, "generation": fmt.Sprint(assignment.Generation)}); err != nil {
		return nil, err
	}
	return affectedNodes, nil
}
