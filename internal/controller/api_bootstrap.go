...
func (s *Server) nodeBootstrap(w http.ResponseWriter, r *http.Request, nodeID string) {
	...
	if !node.Enabled && node.CertificateState != domain.CertificateDecommissioned {
		writeStoreError(w, &domain.ApplyError{Code: "node_disabled", Path: "node", Message: "disabled nodes cannot be provisioned"})
		return
	}
	_, caPEM, err := validateBootstrapConfiguration(s.config, input.ScriptSource)
	if err != nil {
	...
	response, err := buildNodeInstallCommand(s.config, node, platform, arch, input.ScriptSource, plain, caPEM)
	if err != nil {
		writeError(w, http.StatusServiceUnavailable, "bootstrap_unavailable", err.Error())
		return
	}
