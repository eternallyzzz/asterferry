...
func (s *ResourceRepository) createEnrollmentTokenWithOptions(ctx context.Context, nodeID string, ttl time.Duration, options WriteOptions) (string, EnrollmentToken, error) {
	...
	if nodeID != "" {
		...
		if err := tx.QueryRowContext(ctx, `SELECT enabled,certificate_state FROM nodes WHERE id=?`, nodeID).Scan(&enabled, &certificateState); err != nil {
			if errors.Is(err, sql.ErrNoRows) {
				return "", EnrollmentToken{}, ErrNodeNotEnrolled
			}
			return "", EnrollmentToken{}, err
		}
		// A decommissioned identity is the one deliberate exception to the
		// normal enabled-node rule: an administrator may issue a one-time
		// replacement token so the same machine can enroll a fresh certificate
		// without uninstalling the daemon. Disabled/revoked identities still
		// require an explicit repair or a new Node ID.
