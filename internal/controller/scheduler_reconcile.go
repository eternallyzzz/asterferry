...
func (s *Scheduler) reconcileAssignmentSet(ctx context.Context, assignments []domain.Assignment, offlineAfter time.Duration) (result []domain.Assignment, returnErr error) {
	...
	for _, assignment := range assignments {
		...
		nodeUnavailable := !gateway.Enabled || gateway.CertificateState == domain.CertificateRevoked || gateway.CertificateState == domain.CertificateExpired || gateway.CertificateState == domain.CertificateDecommissioned
