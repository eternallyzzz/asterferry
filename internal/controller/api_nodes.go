...
func (s *Server) nodeAction(w http.ResponseWriter, r *http.Request) {
	...
	if len(parts) == 1 && r.Method == http.MethodDelete {
		...
		// DELETE is a lifecycle operation, not a raw row delete. Retain a
		// tombstone so Services and Assignments stay recoverable, revoke the
		// old identity, quarantine its placements, and let the connected Node
		// exit cleanly. A replacement enrollment can later reuse this identity.
		if _, err := s.resources.DecommissionNode(r.Context(), nodeID, WriteOptions{IfMatch: expected, Actor: user.Username, IdempotencyKey: r.Header.Get("Idempotency-Key")}); err != nil {
			writeStoreError(w, err)
			return
		}
		if delivered, err := s.resources.PublishAction(r.Context(), nodeID, "decommission", ""); err != nil {
			slog.Default().Error("failed to publish node decommission action", "node_id", nodeID, "error", err)
		} else if !delivered {
			slog.Default().Warn("node decommission action is queued for next connection", "node_id", nodeID)
		}
	...
	if len(parts) == 3 && parts[1] == "actions" && r.Method == http.MethodPost {
		...
		if action == "purge" {
			if user.Role != RoleAdmin {
				writeError(w, http.StatusForbidden, "forbidden", "permanent node deletion requires an administrator")
				return
			}
			expected, parseErr := parseIfMatch(r.Header.Get("If-Match"))
			if parseErr != nil {
				writeError(w, http.StatusPreconditionRequired, "if_match_required", parseErr.Error())
				return
			}
			if purgeErr := s.resources.PurgeNode(r.Context(), nodeID, WriteOptions{IfMatch: expected, Actor: user.Username, IdempotencyKey: r.Header.Get("Idempotency-Key")}); purgeErr != nil {
				writeStoreError(w, purgeErr)
				return
			}
			// A decommission action may have been queued while the identity was
			// still retained. Publish it once more after the purge so a stream
			// that is still online cannot continue running with a deleted identity.
			if delivered, deliveryErr := s.resources.PublishAction(r.Context(), nodeID, "decommission", ""); deliveryErr != nil {
				slog.Default().Error("failed to publish node purge shutdown action", "node_id", nodeID, "error", deliveryErr)
			} else if !delivered {
				slog.Default().Warn("node purge shutdown action is not currently delivered", "node_id", nodeID)
			}
			w.WriteHeader(http.StatusNoContent)
			return
		}
		if action == "decommission" {
			if user.Role != RoleAdmin {
				writeError(w, http.StatusForbidden, "forbidden", "node decommissioning requires an administrator")
				return
			}
			node, getErr := s.resources.GetNode(r.Context(), nodeID)
			if getErr != nil {
			...
			if _, requestErr := s.resources.DecommissionNode(r.Context(), nodeID, WriteOptions{IfMatch: node.Revision, Actor: user.Username, IdempotencyKey: r.Header.Get("Idempotency-Key")}); requestErr != nil {
				writeStoreError(w, requestErr)
				return
			}
			if delivered, deliveryErr := s.resources.PublishAction(r.Context(), nodeID, action, ""); deliveryErr != nil {
				slog.Default().Error("failed to publish node decommission action", "node_id", nodeID, "error", deliveryErr)
			} else if !delivered {
				slog.Default().Warn("node decommission action is queued for next connection", "node_id", nodeID)
			}
			writeJSON(w, http.StatusAccepted, map[string]any{"node_id": nodeID, "action": action, "requested_by": user.Username, "state": "decommissioned"})
			return
		}
