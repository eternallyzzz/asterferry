...
func TestObservedStorePersistsSystemInfo(t *testing.T) {
	repositories, err := openTestRepositories(filepath.Join(t.TempDir(), "observed-system-info.db"))
	if err != nil {
		t.Fatal(err)
	}
	defer repositories.Close()
	ctx := context.Background()
	if err := repositories.Resources.CreateNode(ctx, domain.Node{ID: "node-info", Name: "node-info", Enabled: true}, WriteOptions{}); err != nil {
		t.Fatal(err)
	}
	usage := 42.25
	observed := domain.ObservedState{SchemaVersion: domain.CurrentControlProtocolVersion, NodeID: "node-info", Healthy: true, SystemInfo: &domain.SystemInfo{Hostname: "host-info", OS: "linux", Architecture: "amd64", CPUUsagePercent: &usage, CollectedAt: time.Now().UTC()}, ObservedAt: time.Now().UTC()}
	document, err := json.Marshal(observed)
	if err != nil {
		t.Fatal(err)
	}
	if err := repositories.Resources.SaveObserved(ctx, ObservedRecord{NodeID: observed.NodeID, Generation: 0, Document: document, UpdatedAt: observed.ObservedAt}); err != nil {
		t.Fatal(err)
	}
	loaded, err := repositories.Resources.GetObserved(ctx, observed.NodeID)
	if err != nil {
		t.Fatal(err)
	}
	if loaded.SystemInfo == nil || loaded.SystemInfo.Hostname != "host-info" || loaded.SystemInfo.CPUUsagePercent == nil || *loaded.SystemInfo.CPUUsagePercent != usage {
		t.Fatalf("stored system info = %#v", loaded.SystemInfo)
	}
}
