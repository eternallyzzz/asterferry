...
func (s *ResourceRepository) putNodeSpecDocument(ctx context.Context, spec domain.NodeSpec, options WriteOptions) error {
	...
	if requestSpec.Agent != nil {
		// Keep the request used for the idempotency hash detached from the
		// document that receives the persisted revision below.  A shallow copy
		// would alias Agent and make recordIdempotency hash a different request
		// after spec.Agent.Revision is updated.
		value := *requestSpec.Agent
		requestSpec.Agent = &value
	}
