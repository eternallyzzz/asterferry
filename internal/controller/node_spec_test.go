...
func TestAgentSpecRequiresGatewayBehaviorAndProtectsBinding(t *testing.T) {
	...
	if err := store.PutAgentSpec(ctx, domain.AgentSpec{NodeID: "agent", GatewayID: "gateway"}, WriteOptions{IdempotencyKey: "agent-spec-create-once"}); err != nil {
		t.Fatal(err)
	}
	...
	decommissionedGateway, err := store.DecommissionNode(ctx, "gateway", WriteOptions{IfMatch: gatewayNode.Revision})
	if err != nil {
		t.Fatal(err)
	}
	assertApplyError("Gateway node permanent deletion", store.PurgeNode(ctx, "gateway", WriteOptions{IfMatch: decommissionedGateway.Revision}), "resource_conflict")
}
