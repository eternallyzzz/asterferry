...
func TestBackupPublishesAndRestoreVerifiesManifest(t *testing.T) {
	...
	writeBootstrapTestAssets(t, result.Config, "1.0.0", "https://github.com/eternallyzzz/asterferry/releases/download")
	...
	for _, name := range backupNodeInstallerFiles {
		if _, err := os.Stat(filepath.Join(backupDir, name)); err != nil {
			t.Fatalf("backup Node installer asset %q is missing: %v", name, err)
		}
	}
	...
	if restoredConfig.DatabasePath != filepath.Join(destination, "controller.db") || restoredConfig.MasterKeyPath != filepath.Join(destination, "master.key") || restoredConfig.NodeInstallersDir != filepath.Join(destination, "node-installers") {
