...
const (
	...
	// CertificateDecommissioned is a terminal node lifecycle state. The node
	// row is retained as a tombstone when it still owns Services or
	// Assignments, so those resources remain recoverable without allowing the
	// old certificate to reconnect.
	CertificateDecommissioned = "decommissioned"
...
func (n Node) Validate() error {
	...
	return n.ValidateMetadata()
}

// ValidateMetadata validates the mutable identity metadata without requiring
// an ID. It is used only while the Controller allocates an ID for a pending
// installation; enrolled Nodes must always use Validate.
func (n Node) ValidateMetadata() error {
	...
	if n.CertificateState != "" && n.CertificateState != CertificatePending && n.CertificateState != CertificateActive && n.CertificateState != CertificateRevoked && n.CertificateState != CertificateExpired && n.CertificateState != CertificateDecommissioned {
