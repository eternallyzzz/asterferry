...
func (r *Runtime) Run(ctx context.Context) error {
	...
	if marked, err := r.hasDecommissionMarker(); err != nil {
		return fmt.Errorf("check node lifecycle marker: %w", err)
	} else if marked {
		r.logger.Warn("node is decommissioned; explicit re-enrollment is required before starting it")
		return nil
	}
	r.runtimeMu.Lock()
	r.runCtx = ctx
	...
	for {
		...
		if err != nil {
			...
			if isControllerIdentityRejection(err) || errors.Is(err, ErrNodeDecommissioned) {
				...
				if isControllerDecommissioned(err) {
					if markerErr := r.persistDecommissionMarker(err); markerErr != nil {
						r.logger.Error("failed to persist node decommission marker", "error", markerErr)
					}
				}
				if isControllerDecommissioned(err) {
					r.logger.Warn("node has been decommissioned; stopping; run explicit node re-enrollment to recover", "error", err)
				} else {
					r.logger.Warn("Controller rejected node identity; stopping until an operator repairs or re-enrolls it", "error", err)
				}
				return nil
			}
...
func (r *Runtime) runConnection(ctx context.Context) error {
	...
	go func() {
		...
		observedTicker := time.NewTicker(r.runtimeOpts.SystemInfoInterval)
		...
		for {
			...
			case <-observedTicker.C:
				if err := sendObserved(); err != nil {
					r.logger.Warn("node observed state send failed", "error", err)
					cancelConnection()
					return
				}
