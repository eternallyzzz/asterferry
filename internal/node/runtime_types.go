...
func NewRuntime(bootstrap Bootstrap, options RuntimeOptions) (*Runtime, error) {
	...
	if options.SystemInfoInterval <= 0 {
		options.SystemInfoInterval = 30 * time.Second
	}
	if options.SystemInfoCollector == nil {
		options.SystemInfoCollector = systeminfo.New(systeminfo.Options{ServiceMode: options.ServiceMode, DiskPath: filepath.Dir(cachePath)})
	}
	...
	runtime := &Runtime{bootstrap: bootstrap, runtimeOpts: options, logger: options.Logger, bootstrapPath: options.BootstrapPath, systemInfoCollector: options.SystemInfoCollector}
