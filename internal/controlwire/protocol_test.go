...
func TestObservedProtoRoundTripsSystemInfo(t *testing.T) {
	usage := 12.5
	state := domain.ObservedState{
		SchemaVersion: domain.CurrentControlProtocolVersion,
		NodeID:        "node-1",
		...
		SystemInfo: &domain.SystemInfo{
			OS: "linux", Architecture: "amd64", IPAddresses: []string{"192.0.2.10"},
			CPUUsagePercent: &usage, CollectedAt: time.Now().UTC(),
		},
	...
	decoded, err := ObservedFromProto(encoded)
	if err != nil {
		t.Fatal(err)
	}
	if decoded.SystemInfo == nil || decoded.SystemInfo.OS != "linux" || decoded.SystemInfo.CPUUsagePercent == nil || *decoded.SystemInfo.CPUUsagePercent != usage {
		t.Fatalf("system info was not preserved: %#v", decoded.SystemInfo)
	}
}
