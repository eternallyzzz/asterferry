...
- Dashboard installation tasks generate a cryptographically random immutable
  Node ID when the request omits one, and retries with the same idempotency key
  replay that ID without creating another pending identity. Nodes report
  bounded static and dynamic host information in their observed state.
...
- AFDP/1 to AFDP/2 and control/1 to control/3 are intentional wire-level
