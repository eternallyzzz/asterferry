...
func newControllerInitCommand() *cobra.Command {
	var dir, username, password, passwordFile, httpListen, metricsListen, grpcListen, grpcAdvertise string
...
func newControllerConfigureCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Args:  cobra.NoArgs,
		RunE: func(cmd *cobra.Command, _ []string) error {
			config, err := controller.Configure(cmd.Context(), controller.ConfigureOptions{ConfigPath: path, GRPCAdvertise: grpcAdvertise})
			if err != nil {
				return err
			}
...
func newNodeRunCommand() *cobra.Command {
	var bootstrapPath, serviceName, serviceMode string
	...
	cmd.Flags().StringVar(&serviceName, "service-name", "AsterFerry-Node", "Windows service name when launched by the Service Control Manager")
	cmd.Flags().StringVar(&serviceMode, "service-mode", "foreground", "runtime service mode reported in node system information")
	return cmd
}
